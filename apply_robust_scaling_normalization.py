...
from utils.data_utils import compute_robust_class_4_then_normalize
from utils.plot_utils import plot_raw_mean_feature_per_class, plot_robust_normalized_mean_feature_per_class
...
config = {
    ...
    "plots_dir": Path("out"),
    "normalized_dir": Path("data/out/normalized_robust"),
    "class_column": "Class",
    "chip_column": "Chip", 
...
try:
    ...
    print("Applying robust scaling normalization...")
    normalized_data, median_stats, mad_stats = compute_robust_class_4_then_normalize(
        data,
        ...
        save_stats_json=str(config["output_dir"] / "robust_normalization_statistics.json")
    )
    ...
    plot_robust_normalized_mean_feature_per_class(
        normalized_data,
        class_column=config["class_column"],
        save_path=str(config["plots_dir"] / "robust_normalized_train_mean_feature_per_class.png"),
        title='Robust Scaled Mean Feature per Class (Training Data)'
    )

    # Save normalized dataset
    output_path = config["output_dir"] / "Robust_Normalized_FTS-MZI_Matrix.csv"
    normalized_data.to_csv(output_path, index=False)
    print(f"Robust scaled dataset saved to {output_path}")
    
    # Split normalized data by chip and save to data/out/normalized_robust/
    print("\nSplitting robust scaled dataset by chip...")
    ...
    for chip in unique_chips:
        ...
        # Save to data/out/normalized_robust/{chip}.csv
    ...
    print(f"\n✅ Robust scaling normalization completed successfully!")
    print(f"📁 Robust scaled chip files saved in: {config['normalized_dir']}")
    print(f"📊 Total chips processed: {len(unique_chips)}")
    
    # Display normalization statistics summary
    print(f"\n📈 Normalization Statistics Summary:")
    print(f"   • Method: Robust scaling based on Class {config['target_class']} (median & MAD)")
    print(f"   • Features normalized: {len(columns_to_normalize)}")
    print(f"   • Median values range: [{median_stats.min():.4f}, {median_stats.max():.4f}]")
    print(f"   • MAD values range: [{mad_stats.min():.4f}, {mad_stats.max():.4f}]")

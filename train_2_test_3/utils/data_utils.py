...
def combine_denoised_data(*X_arrays):
    """
    Combines multiple denoised datasets along the last axis.

    :param X_arrays: Variable-length list of numpy arrays to be combined.
    :return: Combined numpy array.
    """

    if len(X_arrays) == 0:
        raise ValueError("At least one array must be provided.")

    # Stack all arrays along the last axis, excluding the last two columns
    X_combined = np.concatenate([X[:, :, :-2] for X in X_arrays], axis=2)

    # Extract the last 2 columns from the first array (assuming they are identical across arrays)
    temperature_class = X_arrays[0][:, :, -2:]

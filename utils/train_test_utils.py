...
def train_encoder_decoder(epochs, train_loader, val_loader, optimizer, criterion, scheduler, model_encoder_decoder, device, model_encoder_decoder_name, train_loader_chip4, val_loader_chip4, test_loader_chip4, chip_number=1, noise_factor=0.02):
    ...
    for epoch in range(epochs):
        ...
        total_train_loss = 0
        for (inputs, labels), (inputs_chip4, labels_chip4) in zip(train_loader, train_loader_chip4):

            inputs, labels = inputs.to(device), labels.to(device)
            inputs_chip4, labels_chip4 = inputs_chip4.to(device), labels_chip4.to(device)
            ...
            loss = criterion(denoised_inputs, inputs_chip4)
            loss.backward()
            optimizer.step()
        ...
        total_val_loss = 0
        with torch.no_grad():
            for (inputs, labels), (inputs_chip4, labels_chip4) in zip(val_loader, val_loader_chip4):
                inputs, labels = inputs.to(device), labels.to(device)
                inputs_chip4, labels_chip4 = inputs_chip4.to(device), labels_chip4.to(device)
...
def evaluate_encoder_decoder(model_encoder_decoder, data_loader, device, criterion, test_loader_chip4, label_encoder, model_name, conv_layers, chip_number):
    model_encoder_decoder.eval()
    model_encoder_decoder.to(device)
    total_test_loss = 0
    ...
    with torch.no_grad():
        for (inputs, _), (inputs_chip4, _) in zip(data_loader, test_loader_chip4):
            inputs = inputs.to(device)
            inputs_chip4 = inputs_chip4.to(device)

...
def train_encoder_decoder(epochs, train_loader, val_loader, optimizer, criterion, scheduler, model_encoder_decoder, device, model_encoder_decoder_name):
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            optimizer.zero_grad()
            denoised_output = model_encoder_decoder(inputs)[0] # Extract only the denoised output not latent space (check the class ConvDenoiser architecture)
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                ...
                inputs = inputs.unsqueeze(1)
                labels = labels.unsqueeze(1)
                denoised_output = model_encoder_decoder(inputs)[0]
                loss = criterion(denoised_output, labels)

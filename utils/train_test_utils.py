...
def train_encoder_decoder(epochs, train_loader, val_loader, optimizer, criterion, scheduler, model_encoder_decoder, device, model_encoder_decoder_name, noise_factor=0.02):
...
def evaluate_encoder_decoder(model_encoder_decoder, data_loader, device, criterion):
    model_encoder_decoder.eval()
    model_encoder_decoder.to(device)
    total_test_loss = 0
...
def evaluate_classifier(model_classifier, data_loader, device, label_encoder, model_name):
    ...
    output_dir = "out/classification_reports"
